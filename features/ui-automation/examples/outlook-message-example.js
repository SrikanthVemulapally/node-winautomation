...
async function createOutlookMessage(recipient, subject, body, sendMessage = false) {
  try {
    ...
    let outlookWindow = root.findFirst(
      ...
      automation.createPropertyCondition(UIAutomation.PropertyIds.NamePropertyId, 'Inbox - Outlook')
    );

    if (!outlookWindow) {
      outlookWindow = root.findFirst(
        UIAutomation.TreeScopes.Children,
        automation.createPropertyCondition(UIAutomation.PropertyIds.NamePropertyId, 'Microsoft Outlook')
      );
    }
    ...
    let messageWindow = root.findFirst(
      UIAutomation.TreeScopes.Children,
      automation.createPropertyCondition(UIAutomation.PropertyIds.NamePropertyId, 'Untitled - Message (HTML)')
    );
    ...
    if (!messageWindow) {
      console.log('New message window not found. Trying to click New Email button...');
      
      // Try to find and click New Email button
      const newEmailButton = outlookWindow.findFirst(
        UIAutomation.TreeScopes.Descendants,
        automation.createPropertyCondition(UIAutomation.PropertyIds.NamePropertyId, 'New Email')
      );
      
      if (newEmailButton) {
        console.log('Found New Email button, clicking...');
        const invokePattern = newEmailButton.getCurrentPattern(UIAutomation.PatternIds.InvokePatternId);
        if (invokePattern) {
          invokePattern.invoke();
          await new Promise(r => setTimeout(r, 3000)); // Wait for window to open
          
          // Try to find message window again
          messageWindow = root.findFirst(
            UIAutomation.TreeScopes.Children,
            automation.createPropertyCondition(UIAutomation.PropertyIds.NamePropertyId, 'Untitled - Message (HTML)')
          );
      ...
      if (!messageWindow) {
        console.log('Still could not find message window. Please create a new message manually.');
        return;
      }
    }
    ...
    const toField = messageWindow.findFirst(
      ...
      automation.createPropertyCondition(UIAutomation.PropertyIds.AutomationIdPropertyId, '4100')
    );

    if (toField) {
      toField.setFocus();
      await new Promise(r => setTimeout(r, 300));
      const valuePattern = toField.getCurrentPattern(UIAutomation.PatternIds.ValuePatternId);
      if (valuePattern) {
      ...
      const toFieldAlt = messageWindow.findFirst(
        UIAutomation.TreeScopes.Descendants,
        automation.createPropertyCondition(UIAutomation.PropertyIds.NamePropertyId, 'To')
      );
      if (toFieldAlt) {
        toFieldAlt.setFocus();
        await new Promise(r => setTimeout(r, 300));
        const valuePattern = toFieldAlt.getCurrentPattern(UIAutomation.PatternIds.ValuePatternId);
        if (valuePattern) {
          valuePattern.setValue(recipient);
          await new Promise(r => setTimeout(r, 500));
        }
    ...
    const subjectField = messageWindow.findFirst(
      TreeScopes.Descendants,
      automation.createPropertyCondition(UIAutomation.PropertyIds.AutomationIdPropertyId, '4101')
    );

    if (subjectField) {
      subjectField.setFocus();
      await new Promise(r => setTimeout(r, 300));
      const valuePattern = subjectField.getCurrentPattern(UIAutomation.PatternIds.ValuePatternId);
      if (valuePattern) {
      ...
      const subjectFieldAlt = messageWindow.findFirst(
        UIAutomation.TreeScopes.Descendants,
        automation.createPropertyCondition(UIAutomation.PropertyIds.NamePropertyId, 'Subject')
      );
      if (subjectFieldAlt) {
        subjectFieldAlt.setFocus();
        await new Promise(r => setTimeout(r, 300));
        const valuePattern = subjectFieldAlt.getCurrentPattern(UIAutomation.PatternIds.ValuePatternId);
        if (valuePattern) {
          valuePattern.setValue(subject);
          await new Promise(r => setTimeout(r, 500));
        }
    ...
    if (!bodyControl) {
      ...
      const editControls = messageWindow.findAll(
        UIAutomation.TreeScopes.Descendants,
        automation.createPropertyCondition(UIAutomation.PropertyIds.ControlTypePropertyId, UIAutomation.ControlTypeIds.EditControlTypeId)
      );
    ...
    if (bodyControl) {
      ...
      const valuePattern = bodyControl.getCurrentPattern(UIAutomation.PatternIds.ValuePatternId);
      if (valuePattern) {
    ...
    if (sendMessage) {
      ...
      const sendButton = messageWindow.findFirst(
        UIAutomation.TreeScopes.Descendants,
        automation.createPropertyCondition(UIAutomation.PropertyIds.NamePropertyId, 'Send')
      );

      if (sendButton) {
        console.log('Clicking Send button...');
        const invokePattern = sendButton.getCurrentPattern(UIAutomation.PatternIds.InvokePatternId);
        if (invokePattern) {
          invokePattern.invoke();
